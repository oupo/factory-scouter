...
export class FactoryHelper {
    public static choose_entry(prng: PRNG, battle_index: number): [PRNG, Entry] {
        let prngp = prng.dup();
        let x = FactoryHelper.choose_entryQ(prngp, battle_index);
        return [prngp, x];
    }

    public static choose_entryQ(prng: PRNG, battle_index: number) {
        let [start, end] = FactoryHelper._choice_range(battle_index);
        let i = end - 1 - prng.randQ(end - start);
        return ALL_ENTRIES[i];
    }

    public static _choice_range(battle_index: number) {
        if (battle_index !== 7) {
            return [0, 150];
        } else {
            return [150, 250];
    ...
    public static choose_entries(prng: PRNG, n: number, battle_index: number, unchoosable: Entry[] = []) {
        let prngp = prng.dup();
        let x = FactoryHelper.choose_entriesQ(prngp, n, battle_index, unchoosable);
        return [prngp, x];
    }

    public static choose_entriesQ(prng: PRNG, n: number, battle_index: number, unchoosable: Entry[] = []) {
        let entries: Entry[] = [];
    ...
    public static choose_starters(prng: PRNG): [PRNG, Entry[]] {
        let prngp = prng.dup();
        let starters = FactoryHelper.choose_startersQ(prngp);
        return [prngp, starters];
    }

    public static choose_startersQ(prng: PRNG) {
    ...
    public static after_consumption(prng: PRNG, entries: Entry[], battle_index: number) {
        let prngp = prng.dup();
        FactoryHelper.after_consumptionQ(prngp, entries, battle_index);
        return prngp;
    }

    public static after_consumptionQ(prng: PRNG, entries: Entry[], battle_index: number) {
